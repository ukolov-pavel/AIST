...
@allure.testcase('https://aj.srvdev.ru/browse/AIST-6833')
class TestDonorCardDocument(BaseTest):

	@allure.step('МД. Документ, удостоверяющий личность (карточка донора в регистратуре донорского отделения) (автотест)')
	@pytest.mark.parametrize('query, test_data_set_number', get_data('data_test_donor_minicard_document.csv'))
	def test_donor_minicard_document(self, query, test_data_set_number):
		...
		assert main_page.get_document_from_minicard() == expected_result
