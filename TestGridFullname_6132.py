...
@allure.testcase('https://aj.srvdev.ru/browse/AIST-6132')
class TestGridFullname(BaseTest):

	@allure.step('МД. Отображение поля "Дата рождения" в гриде (регистратура донорского отделения)')
	@pytest.mark.parametrize('query, test_data_set_number', get_data('data_test_grid_fullname.csv'))
	def test_grid_fullname(self, query, test_data_set_number):
		main_page = DonorsModuleRegistryPage()

		full_query = sql_query(query)

		donorid, fullname = str(full_query[0][0]), full_query[0][1]
