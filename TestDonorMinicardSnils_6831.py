...
@allure.testcase('https://aj.srvdev.ru/browse/AIST-6831')
class TestDonorCardSnils(BaseTest):

	@allure.step('МД. СНИЛС (карточка донора в регистратуре донорского отделения) (автотест)')
	@pytest.mark.parametrize('query, test_data_set_number', get_data('data_test_donor_minicard_snils.csv'))
	def test_donor_minicard_snils(self, query, test_data_set_number):
		...
		assert main_page.get_snils_from_minicard() == expected_result
