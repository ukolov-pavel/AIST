...
@allure.testcase('https://aj.srvdev.ru/browse/AIST-6841')
class TestDonorCardEmail(BaseTest):

	@allure.step('МД. E-mail (карточка донора в регистратуре донорского отделения) (автотест)')
	@pytest.mark.parametrize('query, test_data_set_number', get_data('data_test_donor_minicard_email.csv'))
	def test_donor_minicard_email(self, query, test_data_set_number):
		...
		if expected_result == '':
			assert aistium.element_is_on_the_page(locators_list='minicard_email',element_name='minicard_email') == False
		else:
			assert aistium.get_text(elements=main_page.minicard_email) == expected_result
