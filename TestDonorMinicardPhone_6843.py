...
@allure.testcase('https://aj.srvdev.ru/browse/AIST-6843')
class TestDonorCardPhone(BaseTest):

	@allure.step('МД. Домашний телефон (карточка донора в регистратуре донорского отделения) (автотест)')
	@pytest.mark.parametrize('query, test_data_set_number', get_data('data_test_donor_minicard_phone.csv'))
	def test_donor_minicard_phone(self, query, test_data_set_number):
		...
		if expected_result == '':
			assert aistium.element_is_on_the_page(locators_list='minicard_phone',element_name='minicard_phone') == False
		else:
