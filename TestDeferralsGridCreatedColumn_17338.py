...
@allure.testcase('https://aj.srvdev.ru/browse/AIST-17338')
class TestDeferralsGridCreatedColumn(BaseTest):
    @allure.step('1. МД. Поле Создан в гриде, Отводы донора (карточка донора в регистратуре донорского отделения)')
    @pytest.mark.parametrize('query, test_data_set_number', get_data('data_test_deferrals_grid_created_column.csv'))
    def test_deferral_grid_created_column(self, query, test_data_set_number):
        ...
        main_page.get_grid_values('CreateDate', 1, main_page.deferrals_grid, mode='click')

        assert main_page.get_grid_values('CreateDate', 1, main_page.deferrals_grid, mode='get_value') == expected_result
