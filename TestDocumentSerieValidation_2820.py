...
@allure.testcase('https://aj.srvdev.ru/browse/AIST-2820')
class TestDocumentSerieValidation(BaseTest):

	@allure.step('Проверка поля "Серия" документа при создании донора')
	@pytest.mark.parametrize('document_type, document_serie, document_number, expected_result, test_data_set_number', get_data('data_test_document_serie_validation.tsv', expansion='tsv'))
	def test_document_serie_validation(self, document_type, document_serie, document_number, expected_result, test_data_set_number):
		...
		assert aistium.get_value(locators_list=locators, element_name='document_serie_field_ndp') == expected_result
