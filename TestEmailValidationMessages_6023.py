...
@allure.testcase('https://aj.srvdev.ru/browse/AIST-6023')
class TestEmailValidationMessages(BaseTest):

	@allure.step('Валидация поля Email (Контактная информация) (запрещающие сообщения) при добавлении донора')
	@pytest.mark.parametrize('email, expected_result, test_data_set_number', get_data('data_test_email_validation_messages.csv'))
	def test_email_validation_messages(self, email, expected_result, test_data_set_number):
