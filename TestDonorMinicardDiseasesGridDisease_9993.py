...
@allure.testcase('https://aj.srvdev.ru/browse/AIST-9993')
class TestDonorMinicardDiseasesGridDisease(BaseTest):

	@allure.step('МД. Поле Заболевание в гриде, Заболевания донора (карточка донора в регистратуре донорского отделения)')
	@pytest.mark.parametrize('query, test_data_set_number', get_data('data_test_donor_minicard_diseases_grid_disease.csv'))
	def test_donor_minicard_diseases_grid_desease(self, query, test_data_set_number):
		...
		assert main_page.get_grid_values('DiseaseCode', 1, main_page.diseases_grid) == expected_result
