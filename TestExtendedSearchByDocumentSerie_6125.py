...
@allure.testcase('https://aj.srvdev.ru/browse/AIST-6125')
class TestExtendedSearchByDDocumentSerie(BaseTest):

	@allure.step('1. МД. Поиск донора по Серии документа (расширенный поиск в регистратуре донорского отделения)')
	@pytest.mark.parametrize('document_type, document_serie, expected_result, test_data_set_number', get_data('data_test_extended_search_by_document_serie_a.tsv', expansion='tsv'))
	def test_extended_search_by_document_serie_a(self, document_type, document_serie, expected_result, test_data_set_number):
		...
		assert main_page.get_document_serie_from_extended_search() == expected_result

	@allure.step('2. МД. Поиск донора по Серии документа (расширенный поиск в регистратуре донорского отделения)')
	@pytest.mark.parametrize('document_type, query, test_data_set_number', get_data('data_test_extended_search_by_document_serie_b.csv'))
	def test_extended_search_by_document_serie_b(self, document_type, query, test_data_set_number):
		main_page = DonorsModuleRegistryPage()
