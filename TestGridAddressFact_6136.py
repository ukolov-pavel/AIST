...
@allure.testcase('https://aj.srvdev.ru/browse/AIST-6136')
class TestGridAddressFact(BaseTest):

	@allure.step('МД. Отображение поля Адрес проживания в гриде (регистратура донорского отделения)')
	@pytest.mark.parametrize('query, test_data_set_number', get_data('data_test_grid_address_fact.csv'))
	def test_grid_address_fact(self, query, test_data_set_number):
		main_page = DonorsModuleRegistryPage()

		full_query = sql_query(query)

		donorid, address_fact = str(full_query[0][0]), full_query[0][1]
		...
		assert main_page.get_grid_values('FactAddress', ind, main_page.main_grid) == address_fact
